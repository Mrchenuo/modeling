...
def masked_max(vector: torch.Tensor, mask: torch.Tensor, dim: int, keep_dim: bool = False, min_val: float = -1e7) -> (
...
class PointerNet(nn.Module):
    ...
    def forward(self, input_seq, input_lengths):
        # (batch_size, input_lengths, coordinate)
        if self.batch_first:
        ...
        # (1, 2, batch_size, hidden_size)   **h_n** of shape `(num_layers * num_directions, batch_size, hidden_size)`
        ...
        # ((batch_size, hidden_size), (batch_size, hidden_size))  h_n,c_n:第一个维度为层数，取最后一层，第二个维度双向时为2，取第一个
        tmp_h_n = encoder_h_n[-1, 0, :, :]
        if tmp_h_n.dim() == 3:
            tmp_h_n = tmp_h_n.squeeze(0)
        tmp_c_n = encoder_c_n[-1, 0, :, :]
        if tmp_c_n.dim() == 3:
            tmp_c_n = tmp_c_n.squeeze(0)
        decoder_hidden = (tmp_h_n, tmp_c_n)
        ...
        for i in range(max_seq_len):
            ...
            decoder_input = torch.gather(encoder_outputs, dim=1, index=index_tensor).squeeze(1)  # 找到每个batch中每行最大的（即一个城市序列中此次最大概率访问），即为第一个输出的城市

        # stack是叠加，会增加一个维度
        # t * (batch_size, max_seq_len) t为max_seq_len, stack之后变成(batch_size, max_seq_len, max_seq_len)
        # 原本的pointer_log_scores是t个（batch_size, max_seq_len),t表示timestep,每个时间步输出一个由max_seq_len个seq组成的概率分布。
        # 此例中t等于max_seq_len。所以最终的pointer_log_scores为（batch_size, timestep,max_seq_len), timestep=max_seq_len
